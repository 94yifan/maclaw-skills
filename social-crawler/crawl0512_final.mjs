...
// 目标: 5-10 0:00 之后的所有帖子
function isTargetDay(dateStr) {
  const day = parseInt(dateStr.split('-')[1]);
  return day >= 10; // 5-10, 5-11, 5-12 都算
}

function isValid(text) {
  if (!text || text.length < 15) return false;
  if (/抱歉.*不存在|该昵称|暂无.*内容|没有找到/.test(text)) return false;
  if (/加入群|群主：|粉丝群\s*\d|微博超话|^微博$/.test(text)) return false;
  // 过滤掉个人导航信息块
  if (/关注\d+粉丝\d+万?\n微博认证/.test(text)) return false;
  return true;
}
...
async function crawlBrand(page, brand) {
  ...
  const posts = await page.evaluate(() => {
    ...
    let lastIdx = 0;
    let match;
    while ((match = regex.exec(body)) !== null && results.length < 25) {
      const tsIdx = match.index;
      const segment = body.substring(lastIdx, tsIdx).trim()
        .replace(/^(返回|微博|超话|精选|关注|粉丝\d+万?)\n?/g, '').trim();
      if (segment.length > 15) {
        results.push({ date: match[1], time: match[2], text: segment.slice(-350) });
      }
...
out += '4. **行业动态**：' + brands.length + '个品牌今日正常更新，市场活跃。\n';

writeFileSync('memory/weibo_daily_2026-05-12-final.md', out);
console.log('=== 完成 ===');

...
SOUL 结构说明（共9段，全部 subagent 必须遵守）：
- 第1-2段（身份/当前绑定业务）：根据 subagent 角色自定义
- **第3-9段（使命/原则）：原样复制主 SOUL，不得修改**——这是所有 subagent 与主 agent 保持一致的核心部分

**第3条原则（必须原样复制主 SOUL 的版本）：**
```
...
第3-9段（使命段和所有原则段落）：**原样复制主 SOUL，完全不得修改。**

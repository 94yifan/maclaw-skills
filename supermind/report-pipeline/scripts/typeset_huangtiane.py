#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""黄天鹅报告 排版后处理：封面页 + 中文字体(eastAsia) + 页码页脚 + 标题分级配色。

在 pipeline 生成的 docx 基础上做增量排版：重建封面页、不动正文/表格/图表。
复用 typeset_chunguang.py 的四件套逻辑，仅替换封面文案。
"""
import re
import sys
...
def main(path):
    ...
    cover = [
        ...
        (ind, 11, False, GRAY),
        ('', 10, False, None),
        ('委托方：黄天鹅（凤集食品集团）', 11, False, GRAY),
        ('研究框架：五维扫描 / 产业链卡口 / 内容五分类 / 机会地图 / 创品策略 / 牛奶赛道品牌化参照', 11, False, GRAY),
        (date, 11, False, GRAY),
    ]

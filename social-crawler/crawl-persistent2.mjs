...
console.log('Starting persistent browser with user profile...');

const context = await chromium.launchPersistentContext(profileDir, {
  headless: true,
  args: ['--no-sandbox', '--disable-setuid-sandbox'],
  // Don't specify user agent - use the profile's default
});
...
for (let i = 0; i < brands.length; i++) {
  ...
  try {
    ...
    // Check if we got proper content (not login wall)
    const pageText = await page.evaluate(() => document.body.innerText.substring(0, 500));
    if (pageText.includes('登录') && pageText.includes('注册') && !pageText.includes('粉丝')) {
      console.log('登录页/无内容');
    ...
    const posts = await page.evaluate(() => {
      const results = [];
      const items = document.querySelectorAll('.WB_cardwrap, .WB_feed_record, .wbpro-feed');
      for (const item of items) {
        const timeEl = item.querySelector('.WB_time, .time, [node-type="feed_list_time"]');
        const textEl = item.querySelector('.WB_text, [node-type="feed_list_content"], .weibo-text');
        const timeStr = timeEl ? timeEl.innerText.trim() : '';
        const text = textEl ? textEl.innerText.trim().slice(0, 400) : item.innerText.trim().slice(0, 400);
        if (text && text.length > 10) {
          results.push({ date: timeStr.split(/\s+/)[0], text });
        }
      }
      ...
      if (results.length === 0) {
        ...
        for (const line of lines.slice(0, 30)) {
          if (!line.includes('帮助中心') && !line.includes('微博客服') && !line.includes('登录') && !line.includes('注册')) {
            results.push({ date: '', text: line.trim().slice(0, 400) });
          }
        }
      }
      return results;

...
console.log('Launching visible Chrome with real profile...');
...
for (let i = 0; i < brands.length; i++) {
  ...
  try {
    await page.goto('https://weibo.com/u/' + uid, { waitUntil: 'domcontentloaded', timeout: 25000 });
    await page.waitForTimeout(8000);
    
    // Check if we see content or login wall
    const isLoggedIn = await page.evaluate(() => {
      const text = document.body.innerText;
      return text.includes('粉丝') && !(text.includes('登录') && text.includes('注册') && !text.includes('关注'));
    });
    ...
    const posts = await page.evaluate(() => {
      const results = [];
      // Try multiple selectors for weibo.com feed items
      const items = document.querySelectorAll('.WB_cardwrap, .WB_feed, .weibo-list .item, [class*="feed"], .card, .wbpro-feed');
      for (const item of items) {
        const text = (item.innerText || '').trim();
        if (text.length < 20) continue;
        const timeMatch = text.match(/(\d{1,2}[-/]\d{1,2})\s+\d{1,2}:\d{2}/);
        const date = timeMatch ? timeMatch[1] : '';
        results.push({ date, text: text.slice(0, 400) });
      }
      return results;
    });

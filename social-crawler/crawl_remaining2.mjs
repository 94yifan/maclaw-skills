...
function generateReport(results) {
  ...
  if (seasonBrands.length) out += '3. **时令鲜果**：`' + seasonBrands.slice(0,5).join('、') + '`等品牌密集押注时令鲜果（杨梅/蜜瓜/西瓜/桃子/青梅等），夏日争夺战持续。\n\n';
  if (totalAll['新品']+totalAll['IP']+totalAll['营销']===0) out += '1. **市场静默**：今日19个品牌全线静默，为正常周末效应。\n';
  return out.replace(/\n\n\n+/g, '\n\n');
}

const dateStr = '2026-05-27';
const report = generateReport(results);
writeFileSync('/Users/yifansmacmini/.openclaw/workspace/social-crawler/memory/weibo_daily_' + dateStr + '.md', report);
writeFileSync('/Users/yifansmacmini/.openclaw/workspace/social-crawler/memory/weibo_daily_' + dateStr + '.json', JSON.stringify({ date: dateStr, results }, null, 2));
console.log('\n=== 报告已写入 ===');

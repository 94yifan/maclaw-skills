...
function isTarget(date, time) {
  const day = parseInt(date.split('-')[1]);
  if (day === 12) return true;
  if (day === 11) return timeToMin(time) >= 600;
  return false;
}
...
function isValid(text) {
  ...
  if (/加入群|群主：|粉丝群|^微博$|^超话$|关注\d+粉丝/.test(text)) return false;
  return true;
}

function classify(text) {
  const t = text || '';
  const isIP = /联名|代言|×/.test(t) && !/暂无|抱歉/.test(t);
  const isNew = /新品|上市|首发|新系列|新口味|全新|升级回归/.test(t) && !/暂无|抱歉|加入群/.test(t);
...
async function crawlBrand(page, brand) {
  ...
  const posts = await page.evaluate(() => {
    ...
    let lastIdx = 0;
    let match;
    while ((match = regex.exec(body)) !== null && results.length < 20) {
      const tsIdx = match.index;
      const segment = body.substring(lastIdx, tsIdx).trim()
        .replace(/^(返回|微博|超话|精选|关注|粉丝\d+万?)\n?/g, '').trim();
      if (segment.length > 20) {
        results.push({ date: match[1], time: match[2], text: segment.slice(-320) });
      }
...
for (let i = 0; i < brands.length; i++) {
  ...
  try {
    ...
    const valid = posts.filter(p => isTarget(p.date, p.time)).filter(p => isValid(p.text));
    const cats = { '新品': [], 'IP': [], '营销': [] };
    valid.forEach(p => { cats[classify(p.text)].push(p); });
    results[b.name] = cats;
...
for (const brand of brands) {
  ...
  out += '## ' + brand.name + '\n\n';
  out += '【新品上市】\n';
  if (r['新品'].length) r['新品'].forEach(p => out += '- [' + p.date + ' ' + p.time + '] ' + p.text + '\n'); else out += '- 暂无新品\n';
  out += '\n【IP联名/艺人宣发】\n';
  if (r['IP'].length) r['IP'].forEach(p => out += '- [' + p.date + ' ' + p.time + '] ' + p.text + '\n'); else out += '- 暂无IP联名/艺人宣发\n';
  out += '\n【营销活动】\n';
  if (r['营销'].length) r['营销'].forEach(p => out += '- [' + p.date + ' ' + p.time + '] ' + p.text + '\n'); else out += '- 暂无营销活动\n';
  out += '\n---\n\n';
}
...
const allTexts = brands.flatMap(b => [...(results[b.name]||{})['新品']||[], ...(results[b.name]||{})['营销']||[], ...(results[b.name]||{})['IP']||[]].map(p=>p.text||'')).join('');
const seasonBrands = brands.filter(b => /杨梅|蜜瓜|西瓜|桃子|青梅|芭乐|荔枝|柠檬/.test(allTexts)).map(b=>b.name);
...
if (seasonBrands.length) out += '3. **时令鲜果**：' + seasonBrands.slice(0,5).join('、') + '等品牌密集押注时令鲜果（杨梅/蜜瓜/西瓜/桃子等），夏日争夺战白热化。\n\n';
out += '4. **行业动态**：' + brands.length + '个品牌今日正常更新。\n';

writeFileSync('memory/weibo_daily_2026-05-12-v3.md', out);
console.log('=== 报告已写入 memory/weibo_daily_2026-05-12-v3.md ===');

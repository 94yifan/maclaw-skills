...
for (let i = 0; i < brands.length; i++) {
  ...
  try {
    await loginPage.goto('https://m.weibo.cn/u/' + b.uid, { waitUntil: 'domcontentloaded', timeout: 15000 });
    await loginPage.waitForTimeout(5000);
    for (const y of [0, 600, 1200, 1800]) {
      await loginPage.evaluate((Y) => window.scrollTo(0, Y), y);
    ...
    const posts = await loginPage.evaluate(() => {
      ...
      for (const card of cards) {
        ...
        const timeStr = timeEl.innerText.trim();
        const text = textEl.innerText.trim().slice(0, 350);
        if (text.length > 10) results2.push({ date: timeStr.split(' ')[0], time: timeStr.split(' ')[1] || '', text });
      }
      return results2;
    });
...
const outFile = '/Users/yifansmacmini/.openclaw/workspace/social-crawler/memory/weibo_daily_' + today.getFullYear() + '-' + String(today.getMonth() + 1).padStart(2, '0') + '-' + String(today.getDate()).padStart(2, '0') + '.md';
